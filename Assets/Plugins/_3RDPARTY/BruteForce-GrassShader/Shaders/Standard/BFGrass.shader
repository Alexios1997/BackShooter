	...
	Properties
	{
		...
		SubShader
		{
			...
			#pragma multi_compile _ LIGHTMAP_ON
				...
				half4 frag(g2f i) : SV_Target
				{
					...
					half4 col = _MainTex.Sample(my_linear_repeat_sampler, i.uv + dis.xy*0.09);
					half4 colGround = _GroundTex.Sample(my_linear_repeat_sampler, i.uv + dis.xy*0.05);
		...
		struct SHADOW_VERTEX
		{
			...
			float4 frag(g2f i) : SV_Target
			{
				...
				float3 noise = _Noise.Sample(my_linear_repeat_sampler, i.uv * _TilingN2 + dis.xy) * _NoisePower;
				float3 grassPattern = _GrassTex.Sample(my_linear_repeat_sampler, i.uv * _TilingN1 + dis.xy);
				half3 NoGrass = _NoGrassTex.Sample(my_linear_repeat_sampler, i.uv + dis.xy * 0.05);
				NoGrass.r = saturate(NoGrass.r + ripplesG);
