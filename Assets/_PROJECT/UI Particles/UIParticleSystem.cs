...
public class UIParticleSystem : MaskableGraphic {
	...
	protected bool Initialize() {
		...
		if (_particleSystem == null) {
			...
			var pRenderer = _particleSystem.GetComponent<ParticleSystemRenderer>();
			if (pRenderer == null) {
				pRenderer = _particleSystem.gameObject.AddComponent<ParticleSystemRenderer>();
			}
			var currentMaterial = pRenderer.sharedMaterial;
			if (currentMaterial && currentMaterial.HasProperty("_MainTex")) {
				particleTexture = currentMaterial.mainTexture;
			}
			var material = new Material(Shader.Find("UI/Particles/Additive")); // TODO - You should create this discard shader
...
#pragma warning restore 618
			...
			var pColor = particle.GetCurrentColor(_particleSystem);
